...
class DeployControllerIntegrationTests {
	...
	void deployCheckReturnsMarker() throws Exception {
		mockMvc.perform(get("/api/planwith-fo-comment/deploy-check"))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.service").value("planwith-fo-comment"))
				.andExpect(jsonPath("$.marker").value("planwith-fo-comment-deploy-v1"))
				.andExpect(jsonPath("$.message").value("planwith-fo-comment deploy pipeline ok"));
	}

	@Test
	void loginSucceedsWithConfiguredCredentials() throws Exception {
		mockMvc.perform(post("/api/planwith-fo-comment/login")
